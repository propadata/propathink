...
exports = module.exports = internals.Plugin = class {
    ...
    request(requestsArray) {
        ...
        this.propathink._requests[this.name] = {};

        for (let i = 0; i < requestsArray.length; ++i) {

            Console.log('       registering request: ' + requestsArray[i].name);
            Console.table(requestsArray[i]);
            this.propathink._requests[this.name][requestsArray[i].name] = {};
            this.propathink._requests[this.name][requestsArray[i].name] = requestsArray[i];
